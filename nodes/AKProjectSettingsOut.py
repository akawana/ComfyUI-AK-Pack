...
def _load_open_image(vals: dict):
    """Shared logic: load the opened image (from disk or embedded base64)."""
...
class AKProjectSettingsOutImage(io.ComfyNode):
    ...
    def execute(cls, ak_project_settings_json: str) -> io.NodeOutput:
        ...
        image = _load_open_image(vals)

        return io.NodeOutput(image)


class AKProjectSettingsOutFileName(io.ComfyNode):
    @classmethod
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            node_id="AKProjectSettingsOutFileName",
            display_name="AK Project Settings Out File Name",
            category="AK/settings",
            description=(
                "Outputs the output filename and output subfolder path "
    ...
    def execute(cls, ak_project_settings_json: str) -> io.NodeOutput:
        ...
        return io.NodeOutput(output_filename, output_subfolder)
...
class AKProjectSettingsOutExtension(ComfyExtension):
...
async def comfy_entrypoint() -> AKProjectSettingsOutExtension:
    return AKProjectSettingsOutExtension()


NODE_CLASS_MAPPINGS = {
    "AKProjectSettingsOutImage": AKProjectSettingsOutImage,
    "AKProjectSettingsOutFileName": AKProjectSettingsOutFileName,
    "AKProjectSettingsOutResize": AKProjectSettingsOutResize,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "AKProjectSettingsOutImage": "AK Project Settings Out Image",
    "AKProjectSettingsOutFileName": "AK Project Settings Out File Name",
