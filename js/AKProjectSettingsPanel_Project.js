...
export function renderProjectTab(rootEl) {
  ...
  if (enabled.width_height === true) {
    ...
    // Labels row: width [swap] height
    const labelsRow = document.createElement("div");
    labelsRow.style.cssText = "display:flex;align-items:center;justify-content:space-between;gap:6px;";

    const wLabel = document.createElement("div");
    wLabel.textContent = "width";
    wLabel.style.cssText = "flex:1;text-align:center;opacity:0.92;";

    const swapBtn = document.createElement("button");
    swapBtn.type = "button";
    swapBtn.textContent = "◀▶";
    swapBtn.title = "Swap width ↔ height";
    swapBtn.style.cssText = "width:34px;height:30px;border-radius:10px;border:1px solid rgba(255,255,255,0.14);background:rgba(255,255,255,0.06);color:inherit;cursor:pointer;font-size:11px;flex:0 0 auto;";

    swapBtn.addEventListener("mousedown", function (e) {
      e.preventDefault();
      const wVal = clampInt(wCtl.inp.value, 1, 16384);
    ...
    hLabel.style.cssText = "flex:1;text-align:center;opacity:0.92;";

    labelsRow.appendChild(wLabel);
    labelsRow.appendChild(swapBtn);
    labelsRow.appendChild(hLabel);

    // Controls row: [◀] w [▶] [◀] h [▶]
    const controlsRow = document.createElement("div");
    controlsRow.style.cssText = "display:flex;align-items:center;gap:6px;";
    wCtl.wrap.style.flex = "1";
    hCtl.wrap.style.flex = "1";
    controlsRow.appendChild(wCtl.wrap);
    controlsRow.appendChild(hCtl.wrap);
  ...
  if (enabled.open_image === true) {
    ...
    btnPaste.addEventListener("mousedown", async (e) => {
      ...
      const clientSuffix = (app.clientId || "").slice(0, 8);
      const targetName = "paste_" + workflowName + (clientSuffix ? "_" + clientSuffix : "") + "_" + Date.now().toString(36) + "." + ext;
