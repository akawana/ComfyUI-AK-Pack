...
function setNodeBypassed(node, bypassed) {
...
function updateBypassOnBooleanNode(graph, node) {
    ...
    for (const n of nodesInGroup) {
        if (!n) continue;
        setNodeBypassed(n, !enabled);
    }
}

let bypassIntervalStarted = false;

app.registerExtension({
    name: "akawana.BypassOnBoolean",
    ...
    init() {
        ...
        app.queuePrompt = async function (number, batchSize, ...rest) {
            ...
            if (graph) {
                ...
                for (const node of nodes) {
                    if (node?.comfyClass === "BypassOnBoolean") {
                        updateBypassOnBooleanNode(graph, node);
                    }
                }
            }
        ...
        if (!bypassIntervalStarted) {
            bypassIntervalStarted = true;
            setInterval(() => {
                const graph = app.graph;
                if (!graph) return;
                const nodes = graph._nodes || [];
