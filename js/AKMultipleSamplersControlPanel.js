...
import { renderControlPanel, selectNodeByShortcut, clearShortcutCallback, callSeedInc } from "./AKMultipleSamplersControlPanel_Control.js";
import { renderSettingsPanel } from "./AKMultipleSamplersControlPanel_Settings.js";
import { renderGroupsPanel } from "./AKMultipleSamplersControlPanel_Groups.js";
import { renderGroupsSettingsPanel } from "./AKMultipleSamplersControlPanel_GroupsSettings.js";
...
// ── Shortcut manager ──────────────────────────────────────────────────────────
// Ctrl+Shift+1..9 switches the selected sampler.
// The listener is active ONLY while the Samplers tab is open.

let _shortcutActive = false;

function _onShortcutKey(e) {
  if (!e.altKey || e.ctrlKey || e.shiftKey) return;

  // Alt+` — increment seed by 1
  if (e.key === "`") {
    e.preventDefault();
    e.stopPropagation();
    callSeedInc();
  ...
  // Alt+1..9 — select node by shortcut
  const digit = Number(e.key);
  if (!Number.isInteger(digit) || digit < 1 || digit > 9) return;
  e.preventDefault();
  e.stopPropagation();
  // console.log("[MSCP shortcut] keydown Alt+" + digit);
  const result = selectNodeByShortcut(digit, getStore, getControlledNodes);
  // console.log("[MSCP shortcut] selectNodeByShortcut returned:", result);
}

function _enableShortcuts() {
  if (_shortcutActive) return;
  _shortcutActive = true;
  window.addEventListener("keydown", _onShortcutKey, true);
}

function _disableShortcuts() {
  if (!_shortcutActive) return;
  _shortcutActive = false;
  window.removeEventListener("keydown", _onShortcutKey, true);
...
function applyEnabledState(enabled) {
  ...
  if (!on) { _disableShortcuts(); closeThisTab(); }
}
...
function tokenizeNodesList(text) {
  ...
  const SHORTCUT_PREFIX_RE = /^\[([1-9])\]\s*/;
  return raw
    .split(/[\n,;]+/g)
    .map(s => s.trim().replace(SHORTCUT_PREFIX_RE, "").trim())
    .filter(Boolean);
}
...
function renderPanel(el) {
  ...
  const btnSamplers    = mkTabBtn("Samplers");
  const btnGroups      = mkTabBtn("Groups");
  const btnSmpSettings = mkTabBtn("Smp Settings");
  const btnGrpSettings = mkTabBtn("Grp Settings");

  const setActive = (which) => {
    const tabs = { samplers: btnSamplers, groups: btnGroups, smpSettings: btnSmpSettings, grpSettings: btnGrpSettings };
    for (const [k, btn] of Object.entries(tabs)) {
      ...
      btn.style.borderColor  = active ? "rgba(255,255,255,0.24)" : "rgba(255,255,255,0.14)";
    }

    if (which === "samplers") _enableShortcuts();
    else _disableShortcuts();

    content.innerHTML = "";
    if (which === "samplers")         renderControlPanel(content, { getStore, getControlledNodes });
    else if (which === "groups")      renderGroupsPanel(content);
    else if (which === "smpSettings") renderSettingsPanel(content);
    else if (which === "grpSettings") renderGroupsSettingsPanel(content);
  };

  btnSamplers.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); setActive("samplers"); });
  btnGroups.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); setActive("groups"); });
  btnSmpSettings.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); setActive("smpSettings"); });
  btnGrpSettings.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); setActive("grpSettings"); });

  tabsBar.appendChild(btnSamplers);
  tabsBar.appendChild(btnGroups);
  tabsBar.appendChild(btnSmpSettings);
  tabsBar.appendChild(btnGrpSettings);
...
function registerSettings(applyFn) {
  ...
  S.addSetting({
    ...
    name: "Multiple Samplers Control",
    type: "boolean",
    defaultValue: true,
    category: ["AK", "Project, Samplers, Debug Panels", "Multiple Samplers Control"],
