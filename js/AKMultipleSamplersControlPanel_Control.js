import { app } from "../../scripts/app.js";

const LABEL_FONT_SIZE_PX = 14;
const STORE_KEY = "ak_multiple_samplers_control_panel";

const DEBUG_LOGS = false;

// ── Shortcut callback ─────────────────────────────────────────────────────────
// renderControlPanel registers a function here so the shortcut handler in
// the main panel file can call it directly — no querySelector needed.
let _shortcutSelectCallback = null;
let _shortcutSeedIncCallback = null;

export function registerShortcutCallback(fn) {
  _shortcutSelectCallback = typeof fn === "function" ? fn : null;
}

export function clearShortcutCallback() {
  _shortcutSelectCallback = null;
  _shortcutSeedIncCallback = null;
}

export function registerSeedIncCallback(fn) {
  _shortcutSeedIncCallback = typeof fn === "function" ? fn : null;
}

export function callSeedInc() {
  if (typeof _shortcutSeedIncCallback === "function") _shortcutSeedIncCallback();
}

const FIELD_ALIASES = {
  seed: ["seed", "seed_value"],
};

const FIELD_SPECS = [
  { key: "seed", type: "number", step: 1 },
  { key: "steps", type: "number", step: 1 },
  { key: "cfg", type: "number", step: 0.1, ctrlClickValue: 5 },
  { key: "sampler_name", type: "select" },
  { key: "scheduler", type: "select" },
  { key: "denoise", type: "number", step: 0.01, ctrlClickValue: 1 },
  { key: "cycle", type: "number", step: 1, min: 0, label: "detailer_cycle" },
  { key: "guide_size", type: "select", label: "detailer_guide", fixedValues: ["512", "768", "1024", "1280", "1536", "2048"], defaultValue: "768" },
];
const FIXED_2_DECIMALS_KEYS = new Set(["cfg", "denoise"]);

// ── In-memory favorites (session only, cleared on page refresh) ───────────────
const _favorites = []; // Array of { nodeId, snapshot: {seed,steps,cfg,...} }

// After DEBUG_LOGS line, add:
let _ctrlHeld = false;
window.addEventListener("keydown", (e) => { if (e.key === "Control") _ctrlHeld = true; }, true);
window.addEventListener("keyup", (e) => { if (e.key === "Control") _ctrlHeld = false; }, true);
window.addEventListener("blur", () => { _ctrlHeld = false; });
function isCtrlHeld() { return _ctrlHeld; }

function decimalsFromStep(step) {
  const s = Number(step);
  if (!Number.isFinite(s) || s === 0) return 0;
  const txt = String(s);
  const idx = txt.indexOf(".");
  if (idx < 0) return 0;
  return Math.min(10, txt.length - idx - 1);
}

function decimalsForKey(key, step) {
  if (FIXED_2_DECIMALS_KEYS.has(String(key))) return 2;
  return decimalsFromStep(step);
}

function formatNumberForKey(key, num, step) {
  const n = Number(num);
  if (!Number.isFinite(n)) return String(num ?? "");
  const d = decimalsForKey(key, step);
  if (d > 0) return n.toFixed(d);
  return String(Math.trunc(n));
}



function sanitizeCfgStep(st) {
  const v = Number(st?.cfg_step);
  return Number.isFinite(v) && v > 0 ? v : 0.5;
}

function sanitizeDenoiseStep(st) {
  const v = Number(st?.denoise_step);
  return Number.isFinite(v) && v > 0 ? v : 0.05;
}

function sanitizeChangeDelay(st) {
  const v = Number(st?.change_delay);
  if (!Number.isFinite(v) || v < 0) return 300;
  return Math.trunc(v);
}
function extractSelectValuesFromWidget(w) {
  const opts = w?.options;
  if (!opts) return [];
  const v = opts.values;
  if (Array.isArray(v)) return v;
  if (v && typeof v === "object") {
    const arr = Array.isArray(v.items) ? v.items : null;
    if (arr) return arr;
  }
  if (Array.isArray(opts)) return opts;
  return [];
}

function safeToString(v) {
  try { return String(v); } catch (_) { return ""; }
}

function mkLabel(text) {
  const l = document.createElement("div");
  l.textContent = text;
  l.style.fontSize = `${LABEL_FONT_SIZE_PX}px`;
  l.style.opacity = "0.85";
  l.style.margin = "10px 0 6px";
  return l;
}

function mkSelect() {
  const s = document.createElement("select");
  s.style.width = "100%";
  s.style.boxSizing = "border-box";
  s.style.padding = "6px 8px";
  s.style.borderRadius = "8px";
  s.style.border = "1px solid rgba(255,255,255,0.14)";
  s.style.background = "rgba(0,0,0,0.35)";
  s.style.color = "rgba(255,255,255,0.92)";
  return s;
}

function mkRow() {
  const r = document.createElement("tr");
  r.style.margin = "0";
  return r;
}

function mkKeyLabel(text) {
  const td = document.createElement("td");
  td.textContent = text;
  td.style.fontSize = `${LABEL_FONT_SIZE_PX}px`;
  td.style.opacity = "0.85";
  td.style.paddingRight = "8px";
  td.style.paddingTop = "6px";
  td.style.paddingBottom = "6px";
  td.style.maxWidth = "50%";
  td.style.wordBreak = "break-word";
  td.style.verticalAlign = "middle";
  return td;
}

function mkInputText() {
  const i = document.createElement("input");
  i.type = "text";
  i.inputMode = "decimal";
  i.style.width = "100%";
  i.style.boxSizing = "border-box";
  i.style.padding = "6px 8px";
  i.style.borderRadius = "8px";
  i.style.border = "1px solid rgba(255,255,255,0.14)";
  i.style.background = "rgba(0,0,0,0.35)";
  i.style.color = "rgba(255,255,255,0.92)";
  i.style.outline = "none";
  return i;
}

function mkSpinBtn(label) {
  const b = document.createElement("button");
  b.type = "button";
  b.textContent = label;
  b.style.width = "28px";
  b.style.minWidth = "28px";
  b.style.height = "30px";
  b.style.borderRadius = "8px";
  b.style.border = "1px solid rgba(255,255,255,0.14)";
  b.style.background = "rgba(0,0,0,0.35)";
  b.style.color = "rgba(255,255,255,0.92)";
  b.style.cursor = "pointer";
  b.style.userSelect = "none";
  b.style.display = "flex";
  b.style.alignItems = "center";
  b.style.justifyContent = "center";
  b.style.padding = "0";
  return b;
}

function mkNumberControl(key, step) {
  const wrap = document.createElement("div");
  wrap.style.display = "flex";
  wrap.style.alignItems = "center";
  wrap.style.gap = "6px";
  wrap.style.width = "100%";

  const dec = mkSpinBtn("◀");
  const input = mkInputText();
  input.style.flex = "1 1 auto";
  input.style.textAlign = "center";
  const inc = mkSpinBtn("▶");

  input.dataset.step = String(step ?? 1);
  if (FIXED_2_DECIMALS_KEYS.has(String(key))) input.dataset.fixedDecimals = "2";

  wrap.appendChild(dec);
  wrap.appendChild(input);
  wrap.appendChild(inc);

  return { wrap, input, dec, inc };
}

function bumpNumberInput(inputEl, dir) {
  const step = Number(inputEl?.dataset?.step ?? "1");
  const s = Number.isFinite(step) && step !== 0 ? step : 1;
  const raw = String(inputEl?.value ?? "").trim();
  const cur = Number(raw);
  const base = Number.isFinite(cur) ? cur : 0;
  // const next = base + dir * s;
  let next = base + dir * s;
  const minVal = Number(inputEl?.dataset?.min ?? "");
  if (Number.isFinite(minVal) && next < minVal) next = minVal;

  const decimals = (() => {
    const fd = Number(inputEl?.dataset?.fixedDecimals ?? "");
    if (Number.isFinite(fd) && fd >= 0) return Math.min(10, Math.trunc(fd));
    return decimalsFromStep(s);
  })();

  const fixed = decimals > 0 ? next.toFixed(decimals) : String(Math.trunc(next));
  inputEl.value = fixed;
}

function styleDarkOptions(selectEl) {
  for (const o of Array.from(selectEl.options)) {
    o.style.background = "#1b1b1b";
    o.style.color = "rgba(255,255,255,0.92)";
  }
}

function findWidgetByName(node, name) {
  const ws = node?.widgets;
  if (!Array.isArray(ws)) return null;

  const aliases = FIELD_ALIASES[name] || [name];

  for (const w of ws) {
    if (!w) continue;
    for (const a of aliases) {
      if (w.name === a) return w;
      if (typeof w.name === "string" && w.name.replace(/\u00A0/g, " ") === a) return w;
    }
  }

  return null;
}

function clampByWidgetOptions(w, v) {
  const opts = w?.options || {};
  const min = typeof opts.min === "number" ? opts.min : null;
  const max = typeof opts.max === "number" ? opts.max : null;
  if (min !== null && v < min) v = min;
  if (max !== null && v > max) v = max;
  return v;
}

function setWidgetValue(node, widgetName, value) {
  const w = findWidgetByName(node, widgetName);
  if (!w) return false;

  if (typeof value === "number") {
    let v = Number.isFinite(value) ? value : 0;
    v = clampByWidgetOptions(w, v);
    if (w.value === v) return true;
    w.value = v;
  } else {
    const v = String(value ?? "");
    if (String(w.value ?? "") === v) return true;
    w.value = v;
  }

  try {
    if (typeof w.callback === "function") w.callback(w.value, app?.canvas, node, null, null);
  } catch (_) { }

  if (widgetName === "guide_size") {
    const wMax = findWidgetByName(node, "max_size");
    if (wMax && wMax.value !== w.value) {
      wMax.value = w.value;
      try {
        if (typeof wMax.callback === "function") wMax.callback(wMax.value, app?.canvas, node, null, null);
      } catch (_) { }
    }
  }

  if (typeof node?.setDirtyCanvas === "function") node.setDirtyCanvas(true, true);
  return true;
}

function getAllGraphNodes() {
  const g = app?.graph;
  const arr = g?._nodes || g?.nodes || [];
  return Array.isArray(arr) ? arr : [];
}

function getNodeById(id) {
  if (id === undefined || id === null || id === "") return null;
  const nid = String(id);
  for (const n of getAllGraphNodes()) {
    if (String(n?.id) === nid) return n;
  }
  return null;
}

function upsertSelectOptions(selectEl, values) {
  const prev = selectEl.value;
  const newVals = Array.isArray(values) ? values.map(v => String(v)) : [];
  const curVals = Array.from(selectEl.options).map(o => String(o.value));
  const same = curVals.length === newVals.length && curVals.every((v, i) => v === newVals[i]);
  if (same) {
    styleDarkOptions(selectEl);
    return;
  }

  selectEl.innerHTML = "";
  for (const v of newVals) {
    const o = document.createElement("option");
    o.value = v;
    o.textContent = v;
    selectEl.appendChild(o);
  }
  styleDarkOptions(selectEl);

  const hasPrev = Array.from(selectEl.options).some(o => o.value === prev);
  if (hasPrev) selectEl.value = prev;
}

function getLocalStoreFallback() {
  const g = app?.graph;
  if (!g) return { selected_sampler_id: "" };
  if (!g.extra) g.extra = {};
  const st = g.extra[STORE_KEY];
  if (!st || typeof st !== "object") {
    g.extra[STORE_KEY] = { nodes_list: "", sorting_mode: "By name", selected_sampler_id: "", run_on_change_panel: false };
    return g.extra[STORE_KEY];
  }
  if (typeof st.selected_sampler_id !== "string") st.selected_sampler_id = "";
  if (typeof st.run_on_change_panel !== "boolean") st.run_on_change_panel = false;
  return st;
}

function dbg(...args) {
  if (!DEBUG_LOGS) return;
  try { console.log("[AKMultipleSamplers][Control]", ...args); } catch (_) { }
}

function markGraphDirty() {
  const g = app?.graph;
  if (!g) return;
  if (typeof g.setDirtyCanvas === "function") g.setDirtyCanvas(true, true);
  if (typeof g.change === "function") g.change();
  if (typeof g._trigger === "function") {
    try { g._trigger("change"); } catch (_) { }
  }
  if (typeof g._version === "number") g._version++;
}


let delayedForceQueueTimer = 0;
let delayedForceQueueReason = "";

function cancelDelayedForceQueue() {
  if (delayedForceQueueTimer) {
    window.clearTimeout(delayedForceQueueTimer);
    delayedForceQueueTimer = 0;
    delayedForceQueueReason = "";
  }
}

function scheduleDelayedForceQueue(reason, delayMs) {
  cancelDelayedForceQueue();
  delayedForceQueueReason = String(reason || "");
  const d = Number(delayMs);
  const ms = Number.isFinite(d) && d >= 0 ? Math.trunc(d) : 300;
  delayedForceQueueTimer = window.setTimeout(() => {
    delayedForceQueueTimer = 0;
    const r = delayedForceQueueReason || reason;
    delayedForceQueueReason = "";
    markGraphDirty();
    if (typeof app?.queuePrompt === "function") {
      try { app.queuePrompt(0); } catch (_) { }
    }
  }, ms);
}
function setOptions(selectEl, nodes, selectedId) {
  const prev = selectEl.value;
  selectEl.innerHTML = "";

  const noneOpt = document.createElement("option");
  noneOpt.value = "";
  noneOpt.textContent = "(none)";
  noneOpt.style.background = "#1b1b1b";
  noneOpt.style.color = "rgba(255,255,255,0.92)";
  selectEl.appendChild(noneOpt);

  for (const n of nodes) {
    const id = n?.id;
    const title = String(n?.title ?? "");
    const opt = document.createElement("option");
    opt.value = String(id ?? "");
    opt.textContent = title ? `${title}  #${id}` : `#${id}`;
    opt.style.background = "#1b1b1b";
    opt.style.color = "rgba(255,255,255,0.92)";
    selectEl.appendChild(opt);
  }

  const want = String(selectedId ?? "");
  const hasWant = Array.from(selectEl.options).some(o => o.value === want);
  selectEl.value = hasWant ? want : (prev && Array.from(selectEl.options).some(o => o.value === prev) ? prev : want);
}

const ICON_FAV = new URL("./img/i_favorites.png", import.meta.url).toString();
const ICON_FAV_RESET = new URL("./img/i_favorites_reset.png", import.meta.url).toString();

function _snapshotLabel(snap) {
  const seed = String(snap.seed ?? "").slice(-6);
  const cfg = formatNumberForKey("cfg", Number(snap.cfg ?? 0), 0.1);
  const den = formatNumberForKey("denoise", Number(snap.denoise ?? 0), 0.01);
  return `${seed}\n${cfg}\n${den}`;
}

function _mkIconBtn(iconUrl, title, fallbackText, onClick) {
  const btn = document.createElement("button");
  btn.type = "button";
  btn.title = title;
  btn.style.cssText = [
    "width:52px;height:52px;",
    "border-radius:8px;",
    "border:1px solid rgba(255,255,255,0.14);",
    `background:rgba(255,255,255,0.06) url("${iconUrl}") no-repeat center/28px 28px;`,
    "cursor:pointer;",
    "flex-shrink:0;",
  ].join("");
  btn.addEventListener("mousedown", (e) => { e.preventDefault(); e.stopPropagation(); onClick(); btn.blur(); });
  return btn;
}

function _mkFavBtn(snap, currentNodeId, onApply) {
  const active = snap.nodeId === currentNodeId;
  const btn = document.createElement("button");
  btn.type = "button";
  btn.title = active ? "Apply favorite" : "Saved for a different node";
  btn.style.cssText = [
    "width:52px;height:52px;",
    "border-radius:8px;",
    "border:1px solid " + (active ? "rgba(255,255,255,0.22)" : "rgba(255,255,255,0.08)") + ";",
    "background:" + (active ? "rgba(255,255,255,0.10)" : "rgba(255,255,255,0.03)") + ";",
    "cursor:" + (active ? "pointer" : "default") + ";",
    "flex-shrink:0;",
    "padding:2px 4px;",
    "font-size:10px;",
    "line-height:1.4;",
    "white-space:pre;",
    "text-align:center;",
    "display:flex;",
    "align-items:center;",
    "justify-content:center;",
    "color:" + (active ? "rgba(255,255,255,0.92)" : "rgba(255,255,255,0.25)") + ";",
    "font-family:monospace;",
    "overflow:hidden;",
    "box-sizing:border-box;",
  ].join("");
  btn.textContent = _snapshotLabel(snap);
  if (active) {
    btn.addEventListener("mousedown", (e) => { e.preventDefault(); e.stopPropagation(); onApply(snap); btn.blur(); });
  }
  return btn;
}

function renderFavoritesBar(container, currentNodeId, applySnapshotFn, addFavFn) {
  container.innerHTML = "";
  container.style.cssText = [
    "display:flex;",
    "flex-wrap:wrap;",
    "gap:6px;",
    "padding:8px 0 4px;",
    "align-items:flex-start;",
  ].join("");

  // Reset button — always first
  const resetBtn = _mkIconBtn(ICON_FAV_RESET, "Reset favorites", "Reset", () => {
    _favorites.length = 0;
    renderFavoritesBar(container, currentNodeId, applySnapshotFn, addFavFn);
  });
  container.appendChild(resetBtn);

  // Existing favorites — between reset and add
  for (const snap of _favorites) {
    const btn = _mkFavBtn(snap, currentNodeId, (s) => {
      applySnapshotFn(s);
    });
    container.appendChild(btn);
  }

  // Add to favorites button — always last
  const addBtn = _mkIconBtn(ICON_FAV, "Add to favorites", "+ Fav", () => {
    addFavFn();
    renderFavoritesBar(container, currentNodeId, applySnapshotFn, addFavFn);
  });
  container.appendChild(addBtn);
}

export function renderControlPanel(el, ctx = null) {
  el.innerHTML = "";
  el.style.padding = "10px";

  const label = mkLabel("Choose Sampler:");
  const select = mkSelect();
  select.classList.add("ak-mscp-sampler-select");

  const fieldsWrap = document.createElement("table");
  fieldsWrap.style.width = "100%";
  fieldsWrap.style.borderCollapse = "collapse";
  fieldsWrap.style.marginTop = "10px";


  const runOnChangeRow = document.createElement("tr");
  const runOnChangeLabelTd = document.createElement("td");
  runOnChangeLabelTd.textContent = "Run on change (CTRL+mouse to ignore)";
  runOnChangeLabelTd.style.fontSize = `${LABEL_FONT_SIZE_PX}px`;
  runOnChangeLabelTd.style.opacity = "0.85";
  runOnChangeLabelTd.style.paddingRight = "8px";
  runOnChangeLabelTd.style.paddingTop = "6px";
  runOnChangeLabelTd.style.paddingBottom = "6px";
  runOnChangeLabelTd.style.maxWidth = "50%";
  runOnChangeLabelTd.style.wordBreak = "break-word";
  runOnChangeLabelTd.style.verticalAlign = "middle";
  const runOnChangeCheckbox = document.createElement("input");
  runOnChangeCheckbox.type = "checkbox";
  runOnChangeCheckbox.style.width = "18px";
  runOnChangeCheckbox.style.height = "18px";
  runOnChangeCheckbox.style.cursor = "pointer";
  const runOnChangeCbTd = document.createElement("td");
  runOnChangeCbTd.style.verticalAlign = "middle";
  runOnChangeCbTd.appendChild(runOnChangeCheckbox);
  runOnChangeRow.appendChild(runOnChangeLabelTd);
  runOnChangeRow.appendChild(runOnChangeCbTd);
  fieldsWrap.appendChild(runOnChangeRow);

  const getStore = typeof ctx?.getStore === "function" ? ctx.getStore : getLocalStoreFallback;
  const getControlledNodes = typeof ctx?.getControlledNodes === "function" ? ctx.getControlledNodes : () => [];

  // Init defaults for fixedValues selects
  const st0 = getStore();
  for (const spec of FIELD_SPECS) {
    if (spec.fixedValues && spec.defaultValue && !st0[spec.key]) {
      st0[spec.key] = spec.defaultValue;
    }
  }

  const getSelectValues = (key) => {
    const spec = FIELD_SPECS.find(s => s.key === key);
    if (spec?.fixedValues) return spec.fixedValues;

    const st = getStore();
    const sel = getNodeById(st.selected_sampler_id);
    const nodes = getControlledNodes();
    const probe = [];
    if (sel) probe.push(sel);
    for (const n of nodes) if (n && n !== sel) probe.push(n);

    for (const n of probe) {
      const w = findWidgetByName(n, key);
      if (!w) continue;

      const vals = extractSelectValuesFromWidget(w);

      if (Array.isArray(vals) && vals.length) return vals;
    }

    return [];
  };

  const fields = {};
  const numBtns = {};
  const rowsByKey = {};
  for (const spec of FIELD_SPECS) {
    const row = mkRow();
    const klabel = mkKeyLabel(spec.label ?? spec.key);
    const controlTd = document.createElement("td");
    controlTd.style.verticalAlign = "middle";
    let control;
    if (spec.type === "select") {
      control = mkSelect();
      control.style.width = "100%";
      upsertSelectOptions(control, getSelectValues(spec.key));
      control.addEventListener("mousedown", () => upsertSelectOptions(control, getSelectValues(spec.key)));
      controlTd.appendChild(control);
      row.appendChild(klabel);
      row.appendChild(controlTd);
      fieldsWrap.appendChild(row);
      fields[spec.key] = control;
      rowsByKey[spec.key] = row;
      continue;
    } else {
      const nc = mkNumberControl(spec.key, spec.step ?? 1);
      if (spec.min !== undefined) nc.input.dataset.min = String(spec.min);
      if (spec.max !== undefined) nc.input.dataset.max = String(spec.max);
      if (spec.ctrlClickValue !== undefined) nc.input.dataset.ctrlClickValue = String(spec.ctrlClickValue);
      control = nc.wrap;
      numBtns[spec.key] = { dec: nc.dec, inc: nc.inc };
      controlTd.appendChild(control);
      row.appendChild(klabel);
      row.appendChild(controlTd);
      fieldsWrap.appendChild(row);
      fields[spec.key] = nc.input;
      rowsByKey[spec.key] = row;
      continue;
    }
  }

  let lastSig = "";
  let raf = 0;

  const syncFromNode = () => {
    const st = getStore();
    const node = getNodeById(st.selected_sampler_id);
    if (!node) return;
    for (const spec of FIELD_SPECS) {
      const w = findWidgetByName(node, spec.key);
      if (spec.type === "select") {
        upsertSelectOptions(fields[spec.key], getSelectValues(spec.key));
        if (w) {
          fields[spec.key].value = String(w.value ?? "");
        } else if (spec.fixedValues && spec.defaultValue) {
          // widget not found on this node — keep default, don't force-write
          if (!fields[spec.key].value) fields[spec.key].value = spec.defaultValue;
        }
        st[spec.key] = String(fields[spec.key].value ?? "");
      } else {
        if (!w) continue;
        const v = w.value;
        const num = typeof v === "number" ? v : Number(v);
        fields[spec.key].value = Number.isFinite(num) ? formatNumberForKey(spec.key, num, spec.step) : String(v ?? "");
        st[spec.key] = fields[spec.key].value;
      }
    }

    // Apply visibility from settings AND widget presence on the selected node
    const showCycle = st.show_detailer_cycle !== false;
    const showGuide = st.show_detailer_guide !== false;
    const hasCycle = !!findWidgetByName(node, "cycle");
    const hasGuideSize = !!findWidgetByName(node, "guide_size");
    if (rowsByKey.cycle) rowsByKey.cycle.style.display = (showCycle && hasCycle) ? "" : "none";
    if (rowsByKey.guide_size) rowsByKey.guide_size.style.display = (showGuide && hasGuideSize) ? "" : "none";
  };

  const refresh = () => {
    raf = 0;
    const st = getStore();
    const nodes = getControlledNodes();

    const node = getNodeById(st.selected_sampler_id);
    const nodeSnap = {};
    if (node) {
      for (const spec of FIELD_SPECS) {
        const w = findWidgetByName(node, spec.key);
        nodeSnap[spec.key] = w ? safeToString(w.value) : "";
      }
    }
    const sig = JSON.stringify({
      list: st.nodes_list ?? "",
      sort: st.sorting_mode ?? "",
      ids: nodes.map(n => n?.id ?? null),
      titles: nodes.map(n => String(n?.title ?? "")),
      sel: st.selected_sampler_id ?? "",
      node: nodeSnap,
      seed: st.seed ?? "",
      steps: st.steps ?? "",
      cfg: st.cfg ?? "",
      sampler_name: st.sampler_name ?? "",
      scheduler: st.scheduler ?? "",
      denoise: st.denoise ?? "",
      cycles: st.cycles ?? "",
      guide_size: st.guide_size ?? "",
      roc: st.run_on_change_panel === true,
      cfg_step: st.cfg_step ?? "",
      denoise_step: st.denoise_step ?? "",
      change_delay: st.change_delay ?? "",
      show_detailer_cycle: st.show_detailer_cycle !== false,
      show_detailer_guide: st.show_detailer_guide !== false,
    });

    if (sig === lastSig) return;
    lastSig = sig;

    setOptions(select, nodes, st.selected_sampler_id);
    runOnChangeCheckbox.checked = st.run_on_change_panel === true;
    const cs = sanitizeCfgStep(st);
    const ds = sanitizeDenoiseStep(st);
    if (fields.cfg && fields.cfg?.dataset) fields.cfg.dataset.step = String(cs);
    if (fields.denoise && fields.denoise?.dataset) fields.denoise.dataset.step = String(ds);
    syncFromNode();
  };

  const scheduleRefresh = () => {
    if (raf) return;
    raf = window.requestAnimationFrame(refresh);
  };

  select.addEventListener("change", () => {
    const st = getStore();
    st.selected_sampler_id = String(select.value ?? "");
    dbg("selectChange", { selected_sampler_id: st.selected_sampler_id });
    syncFromNode();
    scheduleRefresh();
  });

  // Register shortcut callback — lets the shortcut handler select a node
  // directly by id without any DOM querying.
  registerShortcutCallback((nodeId) => {
    // console.log("[MSCP shortcut] callback called nodeId=", nodeId);
    const st = getStore();
    st.selected_sampler_id = String(nodeId);
    select.value = String(nodeId);
    // console.log("[MSCP shortcut] select.value after set=", select.value, "options count=", select.options.length);
    syncFromNode();
    scheduleRefresh();
  });

  // Register seed +1 callback for the ` shortcut.
  registerSeedIncCallback(() => {
    const inp = fields["seed"];
    if (!inp) return;
    bumpNumberInput(inp, 1);
    applyFieldToNode("seed", true);
  });
  runOnChangeCheckbox.addEventListener("change", () => {
    const st = getStore();
    st.run_on_change_panel = runOnChangeCheckbox.checked === true;
    if (st.run_on_change_panel !== true) cancelDelayedForceQueue();
    scheduleRefresh();
  });



  const applyFieldToNode = (key, commitFormat = false) => {
    const st = getStore();
    const node = getNodeById(st.selected_sampler_id);
    const elc = fields[key];
    if (!elc) return;

    dbg("fieldChange", {
      key,
      selected_sampler_id: st.selected_sampler_id,
      hasNode: !!node,
      tag: elc?.tagName,
      value: safeToString(elc?.value),
      commitFormat,
    });

    if (elc.tagName === "SELECT") {
      const v = String(elc.value ?? "");
      st[key] = v;
      if (node) {
        const spec = FIELD_SPECS.find(s => s.key === key);
        // fixedValues selects store numbers — write as number to the widget
        if (spec?.fixedValues) {
          const num = Number(v);
          setWidgetValue(node, key, Number.isFinite(num) ? num : v);
        } else {
          setWidgetValue(node, key, v);
        }
      }
      const reason = `fieldChange:${key}`;
      if (st.run_on_change_panel === true && !isCtrlHeld()) {
        scheduleDelayedForceQueue(reason, sanitizeChangeDelay(st));
      }
      return;
    }

    const raw = String(elc.value ?? "").trim();
    const num = Number(raw);

    // Clamp denoise to [0, 1]
    if (key === "denoise" && Number.isFinite(num)) {
      const clamped = Math.min(1, Math.max(0, num));
      if (clamped !== num) {
        const fixed = formatNumberForKey(key, clamped, FIELD_SPECS.find(s => s.key === key)?.step);
        elc.value = fixed;
        st[key] = fixed;
        if (node) setWidgetValue(node, key, clamped);
        const reason = `fieldChange:${key}`;
        if (st.run_on_change_panel === true && !isCtrlHeld()) {
          scheduleDelayedForceQueue(reason, sanitizeChangeDelay(st));
        }
        return;
      }
    }

    if (Number.isFinite(num)) {
      if (commitFormat && FIXED_2_DECIMALS_KEYS.has(String(key))) {
        const fixed = formatNumberForKey(key, num, FIELD_SPECS.find(s => s.key === key)?.step);
        elc.value = fixed;
        st[key] = fixed;
        if (node) setWidgetValue(node, key, Number(fixed));
      } else {
        st[key] = raw;
        if (node) setWidgetValue(node, key, num);
      }
    } else {
      st[key] = raw;
      if (node) setWidgetValue(node, key, raw);
    }
    const reason = `fieldChange:${key}`;
    if (st.run_on_change_panel === true && !isCtrlHeld()) {
      scheduleDelayedForceQueue(reason, sanitizeChangeDelay(st));
    }
  };


  for (const spec of FIELD_SPECS) {
    if (spec.type !== "number") continue;
    const btn = numBtns[spec.key];
    const inp = fields[spec.key];
    if (!btn || !inp) continue;

    const attachBtnLogs = (elBtn, dir) => {
      elBtn.addEventListener("pointerdown", (ev) => {
        dbg("btnPointerDown", { key: spec.key, dir, buttons: ev?.buttons, pointerType: ev?.pointerType });
      });
      elBtn.addEventListener("mousedown", (ev) => {
        dbg("btnMouseDown", { key: spec.key, dir, buttons: ev?.buttons });
      });
      elBtn.addEventListener("mouseup", (ev) => {
        dbg("btnMouseUp", { key: spec.key, dir, buttons: ev?.buttons });
      });
    };
    attachBtnLogs(btn.dec, -1);
    attachBtnLogs(btn.inc, 1);

    btn.dec.addEventListener("mousedown", () => {
      dbg("btnClick", { key: spec.key, dir: -1 });
      bumpNumberInput(inp, -1);

      inp.focus({ preventScroll: true });
      inp.select();
      applyFieldToNode(spec.key, true);
    });
    btn.inc.addEventListener("mousedown", () => {
      dbg("btnClick", { key: spec.key, dir: 1 });
      bumpNumberInput(inp, 1);

      inp.focus({ preventScroll: true });
      inp.select();
      applyFieldToNode(spec.key, true);
    });
  }

  for (const spec of FIELD_SPECS) {
    const c = fields[spec.key];
    if (!c) continue;
    c.addEventListener("change", () => applyFieldToNode(spec.key, true));
    c.addEventListener("input", () => {
      if (c.tagName === "SELECT") return;
      applyFieldToNode(spec.key, true);
    });
  }

  // Ctrl+click on number input — set to ctrlClickValue (denoise→1.00, cfg→5.00)
  for (const spec of FIELD_SPECS) {
    if (spec.type !== "number" || spec.ctrlClickValue === undefined) continue;
    const inp = fields[spec.key];
    if (!inp) continue;
    inp.addEventListener("click", (e) => {
      if (!e.ctrlKey) return;
      e.preventDefault();
      inp.value = formatNumberForKey(spec.key, spec.ctrlClickValue, spec.step);
      applyFieldToNode(spec.key, true);
    });
  }

  el.appendChild(label);
  el.appendChild(select);
  el.appendChild(fieldsWrap);

  // ── Favorites bar ────────────────────────────────────────────────────────────
  const favBar = document.createElement("div");
  el.appendChild(favBar);

  const getCurrentNodeId = () => String(getStore().selected_sampler_id ?? "");

  const captureSnapshot = () => {
    const st = getStore();
    const snap = { nodeId: getCurrentNodeId() };
    for (const spec of FIELD_SPECS) {
      const el2 = fields[spec.key];
      snap[spec.key] = el2 ? String(el2.value ?? "") : "";
    }
    _favorites.push(snap);
  };

  const applySnapshot = (snap) => {
    const st = getStore();
    // Switch to the saved node
    if (snap.nodeId && snap.nodeId !== st.selected_sampler_id) {
      st.selected_sampler_id = snap.nodeId;
      select.value = snap.nodeId;
    }
    // Apply field values
    for (const spec of FIELD_SPECS) {
      const inp = fields[spec.key];
      if (!inp || snap[spec.key] === undefined) continue;
      inp.value = snap[spec.key];
      applyFieldToNode(spec.key, true);
    }
    scheduleRefresh();
  };

  const rebuildFavBar = () => {
    renderFavoritesBar(favBar, getCurrentNodeId(), applySnapshot, captureSnapshot);
  };

  // Rebuild favbar when node selection changes
  select.addEventListener("change", rebuildFavBar);

  rebuildFavBar();

  scheduleRefresh();

  // const poll = window.setInterval(scheduleRefresh, 1000);
  // const onRemoved = () => {
  //   window.clearInterval(poll);
  //   el.removeEventListener("DOMNodeRemovedFromDocument", onRemoved);
  // };
  // el.addEventListener("DOMNodeRemovedFromDocument", onRemoved);

  const onWindowMouseUp = (e) => {
    if (!el.contains(e.target)) scheduleRefresh();
  };
  const onWindowKeyDown = (e) => {
    if (e.key === "Enter" && !el.contains(e.target)) scheduleRefresh();
  };

  window.addEventListener("mouseup", onWindowMouseUp);
  window.addEventListener("keydown", onWindowKeyDown);

  const onRemoved = () => {
    window.removeEventListener("mouseup", onWindowMouseUp);
    window.removeEventListener("keydown", onWindowKeyDown);
    el.removeEventListener("DOMNodeRemovedFromDocument", onRemoved);
    clearShortcutCallback();
  };
  el.addEventListener("DOMNodeRemovedFromDocument", onRemoved);
}

/**
 * Parse the nodes_list string and return a Map<number, nodeId_string>
 * where key is the shortcut digit (1-9).
 * Only the FIRST occurrence of each digit is kept.
 * For tokens that match multiple nodes the shortcut goes to the first one.
 */
export function buildShortcutMap(getStore, getControlledNodes) {
  const st = getStore();
  const raw = String(st.nodes_list ?? "");
  // console.log("[MSCP shortcut] buildShortcutMap nodes_list=", raw);
  const lines = raw.split(/[\n,;]+/g);

  // Parse [N] prefix from each token
  const SHORTCUT_RE = /^\s*\[([1-9])\]\s*/;
  const parsed = [];
  for (const line of lines) {
    const m = line.match(SHORTCUT_RE);
    if (m) {
      const digit = Number(m[1]);
      const token = line.slice(m[0].length).trim();
      if (token) parsed.push({ digit, token });
    }
  }

  if (!parsed.length) return new Map();

  // Build full controlled nodes list to resolve tokens → node ids
  const allNodes = getControlledNodes();

  function isIntToken(tok) {
    return /^[0-9]+$/.test(tok) && Number.isFinite(Number(tok));
  }

  const byId = new Map(allNodes.map(n => [n?.id, n]));
  const shortcutMap = new Map(); // digit → node_id string
  const usedDigits = new Set();

  for (const { digit, token } of parsed) {
    if (usedDigits.has(digit)) continue; // first occurrence wins

    let foundId = null;
    if (isIntToken(token)) {
      const n = byId.get(token);
      // const n = byId.get(Number(token));
      if (n) foundId = String(n.id);
    } else {
      const sub = token.toLowerCase();
      for (const n of allNodes) {
        const t = String(n?.title ?? n?.type ?? "");
        if (t.toLowerCase().includes(sub)) {
          foundId = String(n.id);
          break; // first match only
        }
      }
    }

    if (foundId !== null) {
      shortcutMap.set(digit, foundId);
      usedDigits.add(digit);
    }
  }

  return shortcutMap;
}

/**
 * Select the node mapped to `digit` in the sampler select element.
 * Returns true if a node was selected.
 */
export function selectNodeByShortcut(digit, getStore, getControlledNodes) {
  // console.log("[MSCP shortcut] selectNodeByShortcut digit=" + digit + " callback=", typeof _shortcutSelectCallback);
  if (typeof _shortcutSelectCallback !== "function") return false;
  const map = buildShortcutMap(getStore, getControlledNodes);
  // console.log("[MSCP shortcut] shortcutMap=", Object.fromEntries(map));
  const nodeId = map.get(digit);
  // console.log("[MSCP shortcut] nodeId for digit " + digit + "=", nodeId);
  if (nodeId === undefined) return false;
  _shortcutSelectCallback(nodeId);
  return true;
}
